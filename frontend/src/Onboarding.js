...
function Onboarding({ currentUser, onComplete }) {
  ...
  return (
    <div style={{ height: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: '#f8f8f8' }}>
      <div style={{ background: 'white', borderRadius: '16px', padding: '32px', width: '400px', display: 'flex', flexDirection: 'column', gap: '24px', boxShadow: '0 4px 24px rgba(0,0,0,0.08)' }}>

        {/* 상단 */}
        <div>
          <p style={{ fontSize: '20px', fontWeight: '600' }}>환영해요! 👋</p>
          <p style={{ fontSize: '13px', color: '#aaa', marginTop: '6px' }}>
            처음 로그인하셨네요! 기본 정보를 입력해주세요.
          </p>
        </div>
        ...
        <div style={{ display: 'flex', gap: '8px' }}>
          {[1, 2].map(s => (
            <div key={s} style={{ flex: 1, height: '4px', borderRadius: '2px', background: s <= step ? '#534AB7' : '#eee' }} />
          ))}
        </div>
        ...
        {step === 1 && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
            <p style={{ fontSize: '13px', fontWeight: '500', color: '#534AB7' }}>1단계 · 기본 정보</p>
        ...
        {step === 2 && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
            <p style={{ fontSize: '13px', fontWeight: '500', color: '#534AB7' }}>2단계 · 아이디 / 비밀번호 설정</p>
        ...
        <div style={{ display: 'flex', gap: '8px' }}>
          {step === 2 && (
            <button onClick={() => setStep(1)}
              style={{ flex: 1, padding: '12px', border: '1px solid #ddd', borderRadius: '8px', background: 'white', cursor: 'pointer', fontSize: '13px', color: '#888' }}>
              이전
            </button>
          )}
          <button
            onClick={() => {
              if (step === 1) { if (validate()) setStep(2) }
              else handleComplete()
            }}
            style={{ flex: 1, padding: '12px', border: 'none', borderRadius: '8px', background: '#534AB7', cursor: 'pointer', fontSize: '13px', color: 'white', fontWeight: '500' }}>
            {step === 1 ? '다음' : '완료하기'}
          </button>
        </div>
